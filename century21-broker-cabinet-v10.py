...
<div style="text-align: center; padding: 15px 0px; background-color: #111111; border-radius: 4px; margin-bottom: 15px;">
    ...
    <p style="color: #FFFFFF; font-size: 0.85rem; margin: 5px 0 0 0; font-family: 'Barlow', sans-serif; font-weight: 600; text-transform: uppercase; letter-spacing: 2px;">Victory Group</p>
...
<div style="padding: 12px; border-radius: 4px; background-color: #1A1A1A; border: 1px solid #2D2D2D; text-align: center;">
    <p style="font-size: 0.8rem; margin: 0; color: #C5A059; font-family: 'Barlow', sans-serif; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;"><b>Версия MVP: 7.0 (Финальный релиз с ИИ-Консультантом)</b></p>
    <p style="font-size: 0.75rem; margin: 5px 0 0 0; color: #CCCCCC; font-family: 'Barlow', sans-serif; line-height: 1.4;">Финальный MVP-релиз CENTURY 21 с интегрированным ИИ-Консультантом по Книге Брокера</p>
</div>
...
# ==============================================================================
    ...
    with st.expander("🚨 АНАЛИЗ И ВЫЯВЛЕНИЕ КРИТИЧЕСКОЙ ОШИБКИ В ИСХОДНОМ ЛИСТЕ «Лист1» (ФИДБЕК БРОКЕРУ)"):
        ...
        with col_bug2:
            ...
            <div style="background-color: #E2EFDA; padding: 20px; border-radius: 4px; border: 1px solid #C6E0B4; border-left: 5px solid #375623; height: 100%;">
                <h5 style="color: #375623; margin-top: 0; text-transform: uppercase; font-size: 0.85rem; font-weight: 800; letter-spacing: 1px;">✅ БЕЗОШИБОЧНЫЙ РАСЧЕТ VICTORY GROUP</h5>
                <p style="margin: 8px 0; font-size: 0.85rem; color: #555;">• Необходимый ВКД: {needed_gci_fp:,.0f} ₽</p>
    ...
    with col_save1:
        ...
        if st.button("💾 Отправить цель в архив Victory Group"):
            if agent_name.strip() == "":
                ...
                st.success(f"✅ Цель агента {agent_name} успешно сохранена/обновлена в архиве офиса Victory Group!")
                st.rerun()

    with col_save2:
        st.markdown("<h4 style='color: #C5A059 !important;'>🔒 Панель Брокера: Личный архив целей</h4>", unsafe_allow_html=True)
        broker_password = st.text_input("Введите пароль брокера для доступа к архиву (пароль по умолчанию: victory):", type="password", key="broker_pass_archive")
        
        if broker_password == "victory":
            ...
            if len(st.session_state['agent_goals']) == 0:
                ...
                st.markdown(f"##### 🏆 Общая сумма амбиций Victory Group: **{total_team_ambitions:,.0f} ₽**")
...
# ==============================================================================
    ...
    if "chat_history_v9" not in st.session_state:
        st.session_state["chat_history_v9"] = [
            ("assistant", "🤖 Приветствую! Я ваш цифровой ИИ-Консультант Victory Group по Системе знаний и технологий CENTURY 21. Я знаю абсолютно всё о Книге Брокера, регламентах адаптации '90 дней', кадровом учете и финансовом планировании. О чем вы хотите спросить сегодня?")
        ]
    ...
    if rag_mode == "📖 Локальный ИИ-эксперт (Книга брокера)":
        ...
        if len(st.session_state["chat_history_v9"]) > 0 and st.session_state["chat_history_v9"][-1][0] == "user":
            ...
            if any(w in last_q for w in ["звон", "норма", "активн", "звонок", "контакт"]):
                ...
                *Рекомендация:* Регулярно используйте наши интерактивные карточки (Flashcards) для тренировки скрипта холодного звонка и отработки возражений!
                ...
                В сети **CENTURY 21** действует строгий регламент корпоративного внешнего вида (дресс-код Victory Group) [1, 2, 13, 86]:
        ...
        Подключите API-ключ вашей нейросети для живого диалога с оригинальными документами Victory Group. 
        """)
...
def show_corporate_footer():
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; color: #888888; font-size: 0.8rem; font-family: 'Barlow', sans-serif;">
        <div><b>CENTURY 21 Victory Group</b> • Корпоративная экосистема брокера v5.0 (Финальный MVP)</div>
