...
st.set_page_config(
    page_title="CENTURY 21 Financial Model (Бюджет Оптимальный) v5",
    page_icon="💼",
    layout="wide",
...
for i in range(24):
    ...
    # 4. РАСХОДЫ - ФОТ + КОРРЕКТИРОВКИ
    agent_payouts = get_override('  ├─ Выплаты агентам (% комиссионных)', str(i+1), base['agent_comm'][i] * (agent_commission_pct / 38.0) * overall_voeronka_factor)
    payouts_agents_list.append(agent_payouts)
    
    salaries_back = get_override('  ├─ Оклады бэк-офиса (оклады)', str(i+1), base['salaries'][i] * (backoffice_salary_mult / 100.0) * salary_mult)
    salaries_backoffice_list.append(salaries_back)
    
    taxes_payroll = get_override('  └─ Налоги на ФОТ оклады', str(i+1), base['salary_taxes'][i] * (backoffice_salary_mult / 100.0) * salary_mult)
...
capex_m0_franchise = get_override('  ├─ Франшиза (Паушальный взнос + Роспатент)', 'Старт', regional_pau_fee + 17000)
...
with tab2:
    ...
    v_m0_agent = get_override('  ├─ Выплаты агентам (% комиссионных)', 'Старт', 0.0)
    v_m0_salary_back = get_override('  ├─ Оклады бэк-офиса (оклады)', 'Старт', 0.0)
    v_m0_salary_taxes = get_override('  └─ Налоги на ФОТ оклады', 'Старт', 0.0)
    v_m0_fot_total = v_m0_agent + v_m0_salary_back + v_m0_salary_taxes
    ...
    p_and_l_decomposed['Старт'] = [
        "", # Доходы
        f"{v_m0_rev_sec:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_prim:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_rent:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_sub:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_overseas:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_other_p:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_mort:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_ins:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_legal:,.0f} ₽".replace(",", " "),
        f"{v_m0_rev_total:,.0f} ₽".replace(",", " "), # Итого доходы
        
        "", # ФОТ
        f"{v_m0_agent:,.0f} ₽".replace(",", " "),
        f"{v_m0_salary_back:,.0f} ₽".replace(",", " "),
        f"{v_m0_salary_taxes:,.0f} ₽".replace(",", " "),
        f"{v_m0_fot_total:,.0f} ₽".replace(",", " "), # Итого ФОТ
        
        "", # OPEX
        f"{v_m0_rent:,.0f} ₽".replace(",", " "),
        f"{v_m0_internet:,.0f} ₽".replace(",", " "),
        f"{v_m0_mobile:,.0f} ₽".replace(",", " "),
        f"{v_m0_kanc:,.0f} ₽".replace(",", " "),
        f"{v_m0_reklama:,.0f} ₽".replace(",", " "),
        f"{v_m0_hh:,.0f} ₽".replace(",", " "),
        f"{v_m0_buh:,.0f} ₽".replace(",", " "),
        f"{v_m0_bank:,.0f} ₽".replace(",", " "),
        f"{v_m0_cleaning:,.0f} ₽".replace(",", " "),
        f"{v_m0_gsm:,.0f} ₽".replace(",", " "),
        f"{v_m0_courier:,.0f} ₽".replace(",", " "),
        f"{v_m0_events:,.0f} ₽".replace(",", " "),
        f"{v_m0_opex_total:,.0f} ₽".replace(",", " "), # Итого OPEX
        
        "", # Франшиза
        f"{v_m0_hq_fix:,.0f} ₽".replace(",", " "),
        f"{v_m0_hq_deal:,.0f} ₽".replace(",", " "),
        f"{v_m0_hq_crm:,.0f} ₽".replace(",", " "),
        f"{v_m0_hq_kc:,.0f} ₽".replace(",", " "),
        f"{v_m0_hq_total:,.0f} ₽".replace(",", " "), # Итого ЦО
        
        f"{v_m0_tax:,.0f} ₽".replace(",", " "), # Налог УСН
        f"{v_m0_opex_tax_total:,.0f} ₽".replace(",", " "), # Итого операционные расходы
        
        "", # CAPEX
        f"{capex_m0_franchise:,.0f} ₽".replace(",", " "), # Франшиза
        f"{capex_m0_renovation:,.0f} ₽".replace(",", " "), # Ремонт
        f"{capex_m0_equipment:,.0f} ₽".replace(",", " "), # Оборудование
        f"{total_capex_m0:,.0f} ₽".replace(",", " "), # Итого CAPEX
        
        f"{-total_capex_m0 - v_m0_opex_tax_total:,.0f} ₽".replace(",", " "), # Чистая прибыль Месяца 0
