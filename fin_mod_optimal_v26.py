...
for i in range(24):
    ...
    agent_payouts_base = (rev_sec + rev_prim + rev_rent + rev_sub + rev_overseas) * (agent_commission_pct / 100.0)
    ...
    reklama_val = get_override('  ├─ Реклама объектов', str(i+1), base_reklama[i] * (marketing_mult / 100.0))
...
if app_mode == "🎯 Презентация и Шаговый конструктор запуска (Sales)":
    ...
    with calc_tab3:
        ...
        for idx, col in enumerate(months):
            ...
            d_leads = 60 * t_agents_idx
            d_meetings = int(d_leads * (conversion_meeting / 100.0))
            d_contracts = int(d_meetings * (conversion_deal / 100.0))
            d_prepayments = d_contracts * (prepayment_rate / 100.0)
            
            d_sec = d_prepayments * (secondary_ratio / 100.0)
            d_prim = d_prepayments * (primary_ratio / 100.0)
            d_rent = base_deals_rent[idx]
            d_sub = base_deals_suburban[idx]
            d_overseas = base_deals_overseas[idx]
            
            d_total_trans = d_sec + d_prim + d_rent + d_sub + d_overseas
            d_other = d_total_trans * 0.005
            
            d_mort = d_contracts * 0.17
            d_ins = d_contracts * 0.10
            d_legal = d_contracts * 0.17
        ...
        col_config_drv = {
            ...
            "Старт": st.column_config.TextColumn(label="Старт (Формула)", disabled=True)
        }
        ...
        edited_drivers_df = st.data_editor(
            df_drivers,
            use_container_width=True,
            disabled=["Категория / Статья", "Старт"],
        ...
        for idx_row in range(len(df_drivers)):
            ...
            for col in df_drivers.columns[2:]: # Skip 'Старт' column to avoid type error
