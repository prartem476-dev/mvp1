...
# Initialize session state for detailed CAPEX Calculator
...
with st.sidebar.expander("🛠️ 5. Регулировка CAPEX", expanded=False):
    ...
    st.number_input("Паушальный взнос, ₽", min_value=150000, max_value=1500000, key="capex_pau_fee", step=10000)
    st.number_input("Роспатент, ₽", min_value=1000, max_value=100000, key="capex_rospatent", step=1000)
    
    st.markdown("**🏢 Помещение офиса:**")
    st.slider("Площадь офиса, м²", min_value=40, max_value=140, key="capex_office_area", step=5)
    st.slider("Норма м² на человека", min_value=4.0, max_value=12.0, key="capex_norm_m2", step=0.5)
    
    st.markdown("**💻 Рабочие места:**")
    st.number_input("Количество мест (чел.)", min_value=5, max_value=50, key="capex_workstations", step=1)
...
if app_mode == "🎯 Презентация и Шаговый конструктор запуска (Sales)":
    ...
    if step == "1. Локация и Масштаб":
        ...
        with col_left:
            ...
            with c2:
                st.slider("Норма м² на человека", min_value=4.0, max_value=12.0, key="capex_norm_m2", step=0.5)
            with c3:
                st.number_input("Рабочие места (чел.)", min_value=5, max_value=50, key="capex_workstations", step=1)
                
            st.markdown("##### 💻 Цены и комплектация оборудования (руб.):")
            c4, c5, c6 = st.columns(3)
            with c4:
                st.number_input("Цена стола, ₽/шт.", min_value=1000, max_value=50000, key="capex_table_price", step=1000)
            with c5:
                st.number_input("Цена стула, ₽/шт.", min_value=1000, max_value=50000, key="capex_stool_price", step=1000)
            with c6:
                st.number_input("Цена компьютера/ПК, ₽/шт.", min_value=10000, max_value=200000, key="capex_computer_price", step=5000)
                
            with st.expander("🛠️ Тонкие настройки цен ремонта и прочих вложений", expanded=False):
                c7, c8 = st.columns(2)
                with c7:
                    st.number_input("Стоимость ремонта, ₽/м²", min_value=1000, max_value=50000, key="capex_renov_price", step=1000)
                    st.number_input("Доп. мебель (шкаф, тумба), ₽", min_value=10000, max_value=1000000, key="capex_add_furniture", step=10000)
                with c8:
                    st.number_input("Стоимость брендирования, ₽/м²", min_value=500, max_value=20000, key="capex_brand_price", step=500)
                    st.number_input("Роутер, принтер, сеть, всего ₽", min_value=5000, max_value=200000, key="capex_router_printer", step=5000)
        ...
        with col_left:
            ...
            s_leads = st.slider(
                "📞 Количество лидов (звонков), % к базе:",
                min_value=50, max_value=200, value=int(scaling_leads), step=10,
                help="Регулирует объем трафика.",
                key="sales_step4_leads"
            )
            if s_leads != scaling_leads:
                st.session_state['scaling_leads'] = s_leads
                st.rerun()
    ...
    calc_tab1, calc_tab2, calc_tab3, calc_tab4, calc_tab5 = st.tabs([
        ...
        "📋 2. Финансовый отчет P&L (₽)",
        "📈 3. Операционная воронка (шт.)",
        "👥 4. Кадровое планирование (чел.)",
        "📝 5. Справочник и Методология"
    ])
    ...
    with calc_tab3:
        ...
        st.dataframe(df_drivers, use_container_width=True, height=450)
        
    with calc_tab4:
        st.markdown("#### 👥 Штатное расписание офиса (чел.)")
        st.dataframe(df_staff, use_container_width=True, height=480)
        
    with calc_tab5:
        st.markdown("""
        ### 📖 Справочник и методология калькулятора CAPEX CENTURY 21
        
        Интерактивный калькулятор разработан для прозрачной сметы стартовых расходов запуска брокерского офиса:
        
        1. **Помещение (Ремонт и Брендинг):**
           * **Ремонт:** Базовый тариф ремонта составляет **10 000 ₽ / м²** согласно стандартам корпоративного оформления. Площадь офиса регулируется от 40 до 140 м² (default 60 м²).
           * **Брендирование:** Фирменные вывески, оформление ресепшена и переговорных зон — тариф составляет **2 000 ₽ / м²**.
           
        2. **Офисная мебель и рабочие места:**
           * Рассчитывается динамически на основе количества рабочих мест (чел.):
             * **Рабочий стол:** тариф **12 000 ₽ / шт.**
             * **Офисный стул:** тариф **7 000 ₽ / шт.**
             * **Компьютер / Монитор:** тариф **45 000 ₽ / шт.**
           * **Доп. мебель:** фиксированный пакет (шкаф, тумба) составляет **150 000 ₽**.
           * **Сеть и оргтехника:** Роутер, принтер, сетевая разводка — **25 000 ₽**.
           
        3. **Франшиза и Роспатент:**
           * **Паушальный взнос:** зависит от региона РФ (авто-подстройка по пресету) и госпошлина за регистрацию договора франчайзинга в Роспатенте (**17 000 ₽**).
        """)
