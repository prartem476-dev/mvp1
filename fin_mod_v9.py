...
region_select = st.sidebar.selectbox(
    ...
    help="Автоматически калибрует базовые параметры: паушальный взнос, средние чеки сделок, стоимость аренды офиса и региональный индекс зарплат бэк-офиса."
)
...
scenario = st.sidebar.selectbox(
    ...
    help="Сценарий 'Гипотеза 1' – поэтапный запуск с умеренными вложениями. 'Бюджет Оптимальный' – агрессивный старт с полной закупкой оборудования и рекламой с 1-го месяца."
)
...
regional_pau_fee = st.sidebar.number_input("Паушальный взнос (с НДС), ₽", min_value=150000, max_value=1000000, value=preset["pau_fee"], step=10000, help="Единоразовый платеж за право открытия представительства под брендом Century 21. Автоматически подстраивается под тип города.")

# 2. Драйверы воронки продаж
st.sidebar.markdown("### 📈 Воронка продаж")
scaling_leads = st.sidebar.slider("Количество лидов (звонков), % к базе", min_value=50, max_value=200, value=100, step=10, help="Позволяет оценить изменение выручки при росте или падении входящих звонков и заявок относительно базовой модели.")
conversion_meeting = st.sidebar.slider("Конверсия лид -> встреча, %", min_value=2.0, max_value=20.0, value=10.0, step=0.5, help="Показатель качества первичного контакта администратора или рекрутера. Какая доля звонков перерастает в очную встречу в офисе.")

default_conversion_deal = 15.0 if "Гипотеза 1" in scenario else 5.0
conversion_deal = st.sidebar.slider("Конверсия встреча -> договор, %", min_value=1.0, max_value=30.0, value=default_conversion_deal, step=0.5, help="Эффективность работы агентов на встречах. Процент личных встреч, завершающихся подписанием эксклюзивного договора.")

# 3. Средние чеки
st.sidebar.markdown("### 💰 Средние комиссии (чеки сделок)")
comm_secondary = st.sidebar.number_input("Вторичный рынок, ₽", min_value=50000, max_value=1000000, value=preset["comm_sec"], step=10000, help="Средний размер агентского вознаграждения со сделки купли-продажи на вторичном рынке недвижимости в данном регионе.")
comm_primary = st.sidebar.number_input("Первичный рынок, ₽", min_value=50000, max_value=1000000, value=preset["comm_prim"], step=10000, help="Средняя комиссия от застройщика за проведение сделки по продаже квартиры в новостройке (первичный рынок).")
comm_suburban = st.sidebar.number_input("Загородная недвижимость, ₽", min_value=50000, max_value=1000000, value=preset["comm_sub"], step=10000, help="Средняя комиссия за продажу земельных участков, дач, загородных домов и коттеджей.")
comm_rent_val = st.sidebar.number_input("Аренда (жилая/коммерческая), ₽", min_value=10000, max_value=200000, value=preset["comm_rent"], step=5000, help="Средняя стоимость комиссии агентства за проведение сделки долгосрочной аренды жилых или коммерческих помещений.")

# 4. ФОТ и комиссия агентов
st.sidebar.markdown("### 👥 Персонал и Выплаты")
agent_commission_pct = st.sidebar.slider("Средний % выплат агентам", min_value=25, max_value=60, value=38, step=1, help="Средневзвешенный процент от комиссии со сделки, выплачиваемый агенту в качестве переменного вознаграждения.")
backoffice_salary_mult = st.sidebar.slider("Индекс окладов бэк-офиса, % к базе", min_value=50, max_value=150, value=100, step=5, help="Множитель для одновременной корректировки фиксированных окладов всей команды бэк-офиса (РОП, HR, юрист, ипотечный брокер).")

# 5. OPEX и Роялти
st.sidebar.markdown("### 🏢 Аренда и Налоги")
office_rent_custom = st.sidebar.number_input("Аренда офиса в месяц, ₽", min_value=10000, max_value=500000, value=preset["rent_base"], step=5000, help="Стоимость ежемесячной аренды и коммунальных платежей за помещение офиса. Подстраивается под средние ставки выбранного региона.")
royalty_deal_custom = st.sidebar.number_input("Роялти за каждую сделку, ₽", min_value=1000, max_value=5000, value=preset["royalty_deal"], step=100, help="Переменная часть платежа в пользу Центрального Офиса Century 21 за проведение и сопровождение каждой транзакции.")
tax_rate = st.sidebar.slider("Ставка налога УСН, %", min_value=3, max_value=15, value=7, step=1, help="Процент налоговых отчислений по упрощенной системе налогообложения от совокупного дохода (базово 6-7% для УСН 'Доходы').")
