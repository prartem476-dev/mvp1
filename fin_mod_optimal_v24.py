...
for i in range(24):
    ...
    def calc_role_salary(role_name, default_count, prop_factor):
        adjusted_default = default_count
        if role_name == 'РОП: категория C' and (i+1) >= 3 and st.session_state.get('add_rop_m3', False):
            adjusted_default += 1
        if role_name == 'Администратор офиса' and (i+1) >= 6 and st.session_state.get('add_admin_m6', False):
            adjusted_default += 1
            
        role_count = get_headcount_override('  ├─ ' + role_name if 'Агент' not in role_name else '  ├─ ' + role_name, str(i+1), adjusted_default)
        default_salary = salaries_back_base * prop_factor
        if adjusted_default > 0:
            return (role_count / adjusted_default) * default_salary
        else:
...
if app_mode == "🎯 Презентация и Шаговый конструктор запуска (Sales)":
    ...
    with calc_tab3:
        ...
        st.markdown("ℹ️ *Здесь настраиваются плановые показатели воронки продаж и конверсии (в штуках) по месяцам 1-24. Изменения мгновенно пересчитают доходы P&L.*")
        ...
        if st.button("🔄 Сбросить ручные изменения воронки", use_container_width=True):
            st.session_state['driver_overrides'] = {}
