...
st.markdown("<div class='main-title'>CENTURY 21 Financial Application (v2)</div>", unsafe_allow_html=True)
st.markdown("<div class='sub-title'>Интерактивная финансовая модель и MVP-калькулятор для клиентов</div>", unsafe_allow_html=True)

# ----------------------------------------------------
# БОКОВАЯ ПАНЕЛЬ С ДРАЙВЕРАМИ И ПАРАМЕТРАМИ
# ----------------------------------------------------

st.sidebar.markdown("<h2 style='color:#C2A24D; text-align:center;'>Панель управления</h2>", unsafe_allow_html=True)
...
with tab2:
    ...
    if openpyxl_available:
        ...
        st.warning("""⚠️ **Экспорт в Excel временно недоступен в облачной демо-версии.**

Для активации экспорта добавьте библиотеку `openpyxl` в ваш файл зависимостей. Поскольку Streamlit Cloud по умолчанию распознает только файл **`requirements.txt`** в корне репозитория, вы можете переименовать ваш файл `req_fin.txt` на GitHub в `requirements.txt` (или скопировать его содержимое туда), и Streamlit установит все библиотеки автоматически!""")
