...
<div style="padding: 12px; border-radius: 4px; background-color: #1A1A1A; border: 1px solid #2D2D2D; text-align: center;">
    <p style="font-size: 0.8rem; margin: 0; color: #C5A059; font-family: 'Barlow', sans-serif; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;"><b>Версия MVP: 29.0 (Премиальная ERP Экосистема C21 v29.0)</b></p>
...
col_main, col_billing = st.columns([3.0, 1.1])
        ...
        if user_role != "🏢 Управляющая Компания (Куратор УК)":
            ...
            with tab_ai_hub:
                ...
                with col_calc2:
                    ...
                    with col_res_ai2:
                        st.metric("Расходы с ИИ-шлюзом v29", f"{cost_routed * 90:,.0f} ₽ / мес.", help="При распределении задач между Gemini Flash, дешевыми и локальными бесплатными моделями.")
...
with col_billing:
    st.markdown("""
    <div style="background-color: #111111; padding: 12px 15px; border-left: 5px solid #C5A059; border-radius: 4px; margin-bottom: 20px;">
        <h3 style="color: #FFFFFF !important; margin: 0; font-size: 1.25rem; font-family: 'Barlow', sans-serif; text-transform: uppercase; letter-spacing: 1px;">💳 БИЛЛИНГ & ФИНАНСЫ</h3>
        <p style="color: #C5A059; margin: 3px 0 0 0; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 1px; font-weight: 600;">Комплаенс-контроль роялти 6%</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Run dynamic billing update
    update_billing_statuses()
    
    # Load billing data from SQLite
    conn = sqlite3.connect(db_path)
    df_billing = pd.read_sql_query("""
        SELECT fb.id, fo.partner_name, fb.invoice_number, fb.period, fb.gci_amount, 
               fb.royalty_amount, fb.due_date, fb.paid_date, fb.status, fb.penalty_amount, fo.telegram_id, fb.franchise_id
        FROM franchise_billing fb
        LEFT JOIN franchise_onboarding fo ON fb.franchise_id = fo.id
    """, conn)
    conn.close()
    ...
    if user_role == "🏢 Управляющая Компания (Куратор УК)":
        ...
        # Premium CSS Card for KPI
        rate_color = "#76933C" if collection_rate >= 90 else "#C65911"
        st.markdown(f"""
        <div style="background-color: #FFFFFF; padding: 12px 15px; border: 1px solid #E2E2E6; border-left: 5px solid #C5A059; border-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); margin-bottom: 15px;">
            <p style="margin: 0; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.5px; color: #666666; font-weight: 700;">Собираемость роялти:</p>
            <h3 style="margin: 3px 0 0 0; font-size: 1.6rem; font-weight: 800; color: {rate_color};">{collection_rate:.1f}%</h3>
            <p style="margin: 3px 0 0 0; font-size: 0.75rem; color: #333;">Собрано: <b>{total_royalties_collected:,.0f} ₽</b> из {total_royalties_invoiced:,.0f} ₽</p>
        </div>
        <div style="background-color: #FFFFFF; padding: 12px 15px; border: 1px solid #E2E2E6; border-left: 5px solid #C65911; border-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); margin-bottom: 15px;">
            <p style="margin: 0; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.5px; color: #666666; font-weight: 700;">Дебиторка сети:</p>
            <h3 style="margin: 3px 0 0 0; font-size: 1.4rem; font-weight: 800; color: #C65911;">{total_debt:,.0f} ₽</h3>
            <p style="margin: 1px 0 0 0; font-size: 0.72rem; color: #777;">Пени (0.1%/д): <b>+ {total_penalties:,.0f} ₽</b></p>
        </div>
        """, unsafe_allow_html=True)
        
        # Debtor list selectbox & verification (Compact layout)
        outstanding_invoices = df_billing[df_billing["status"] != "🟢 Оплачено"]
        
        st.markdown("<p style='font-size:0.85rem; font-weight:700; margin-bottom:5px; color:#111; text-transform:uppercase;'>⚙️ Действия Куратора УК</p>", unsafe_allow_html=True)
        if not outstanding_invoices.empty:
            inv_options = outstanding_invoices.apply(lambda r_row_lbl: f"{r_row_lbl['invoice_number']} ({r_row_lbl['partner_name'].split('(')[0].strip()})", axis=1).tolist()
            selected_inv_label = st.selectbox("Выберите неоплаченный счет:", inv_options, key="uk_side_inv_select")
            
            # Find the actual inv row
            selected_inv_num = selected_inv_label.split(" (")[0]
            ...
            st.markdown(f"<p style='font-size:0.75rem; margin: -5px 0 8px 0; color:#555;'>Долг: <b>{inv_royalty:,.0f} ₽</b> + пени <b>{inv_row['penalty_amount']:,.0f} ₽</b></p>", unsafe_allow_html=True)
            
            if st.button("🟢 Подтвердить оплату", use_container_width=True, key="uk_side_confirm_btn"):
                conn = sqlite3.connect(db_path)
                cursor = conn.cursor()
                cursor.execute("""
                    UPDATE franchise_billing 
                ...
                st.toast(f"Платеж по счету {selected_inv_num} зафиксирован!")
                st.rerun()
        else:
            st.success("🎉 Все счета оплачены!")
            
        # Remind button
        if st.button("🔔 Напомнить должникам", use_container_width=True, key="uk_side_remind_btn"):
            ...
            if not overdue_invs.empty:
                ...
                st.toast("Напоминания отправлены!")
                st.rerun()
            else:
                st.info("Должников в сети не обнаружено.")
                
        # Quick Invoice expander
        with st.expander("➕ Выставить новый счет", expanded=False):
            with st.form("uk_side_invoice_form", clear_on_submit=True):
                conn = sqlite3.connect(db_path)
                cursor = conn.cursor()
                cursor.execute("SELECT id, partner_name FROM franchise_onboarding")
                ...
                selected_partner_id = st.selectbox(
                    ...
                    key="uk_side_partner_sel"
                )
                
                inv_period = st.text_input("Период (напр: Август 2026):", "Август 2026", key="uk_side_inv_period")
                inv_gci = st.number_input("ВКД офиса (руб.):", 50000, 10000000, 1500000, 50000, key="uk_side_inv_gci")
                inv_due_days = st.slider("Срок оплаты (дней):", 3, 30, 10, key="uk_side_inv_due_days")
                
                inv_submitted = st.form_submit_button("🧾 Выставить счет")
                if inv_submitted:
                    conn = sqlite3.connect(db_path)
                    cursor = conn.cursor()
                    cursor.execute("SELECT COUNT(*) FROM franchise_billing")
        ...
        if selected_partner_name:
            ...
            if partner_outstanding.empty:
                st.markdown("""
                <div style="background-color: #E2EFDA; padding: 12px 15px; border: 1px solid #C5A059; border-left: 5px solid #375623; border-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); margin-bottom: 15px; text-align: center;">
                    <p style="margin: 0; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.5px; color: #375623; font-weight: 700;">Статус счета:</p>
                    <h3 style="margin: 3px 0 0 0; font-size: 1.25rem; font-weight: 800; color: #375623;">🟢 ДОЛГОВ НЕТ</h3>
                    <p style="margin: 3px 0 0 0; font-size: 0.7rem; color: #444;">Все обязательства по роялти выполнены!</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                total_outstanding_royalty = partner_outstanding["royalty_amount"].sum()
                total_outstanding_penalty = partner_outstanding["penalty_amount"].sum()
                total_to_pay = total_outstanding_royalty + total_outstanding_penalty
                
                has_overdue = not partner_outstanding[partner_outstanding["status"] == "🔴 Просрочено"].empty
                card_border = "#C65911" if has_overdue else "#C5A059"
                card_color = "#C65911" if has_overdue else "#C5A059"
                
                st.markdown(f"""
                <div style="background-color: #FFFFFF; padding: 12px 15px; border: 1px solid #E2E2E6; border-left: 5px solid {card_border}; border-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); margin-bottom: 15px; text-align: center;">
                    <p style="margin: 0; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.5px; color: #666666; font-weight: 700;">К уплате роялти:</p>
                    <h3 style="margin: 3px 0 0 0; font-size: 1.5rem; font-weight: 800; color: {card_color};">{total_to_pay:,.0f} ₽</h3>
                    <p style="margin: 3px 0 0 0; font-size: 0.7rem; color: #555;">Роялти: {total_outstanding_royalty:,.0f} ₽ | Пеня: {total_outstanding_penalty:,.0f} ₽</p>
                </div>
                """, unsafe_allow_html=True)
                
            # Payment simulation
            if not partner_outstanding.empty:
                st.markdown("<p style='font-size:0.85rem; font-weight:700; margin-bottom:5px; color:#111; text-transform:uppercase;'>💳 Быстрая оплата СБП</p>", unsafe_allow_html=True)
                selected_pay_inv = st.selectbox("Выберите счет для оплаты:", partner_outstanding["invoice_number"].tolist(), key="broker_side_inv_sel")
                ...
                sum_to_pay = pay_royalty + pay_penalty
                
                if st.button("💳 Оплатить роялти онлайн", use_container_width=True, key="broker_side_pay_btn"):
                    conn = sqlite3.connect(db_path)
                    cursor = conn.cursor()
                    cursor.execute("""
                        UPDATE franchise_billing 
                    ...
                    log_message(f"💳 [ПЛАТЕЖ ОТПРАВЛЕН] Франчайзи {selected_partner_name} произвел онлайн-оплату счета {selected_pay_inv} на сумму {sum_to_pay:,.0f} ₽.")
                    log_message(f"📱 [PUSH] Чек отправлен {pay_telegram}: '[ОПЛАТА ВЫПОЛНЕНА 💳] Платеж по счету {selected_pay_inv} на сумму {sum_to_pay:,.0f} ₽ отправлен в банк! Статус счета изменен на «Оплачено».'")
                    st.toast("Оплата произведена! Реестр обновлен.")
                    st.rerun()
            
            with st.expander("🧾 История начислений", expanded=False):
                if not df_partner_billing.empty:
                    ...
                    st.dataframe(df_partner_display.rename(columns={
                        "invoice_number": "Счет",
                        "period": "Период",
                        "royalty_amount": "Роялти",
                        "due_date": "Срок",
                        "paid_date": "Оплачено",
                        "status": "Статус"
                    })[["Счет", "Период", "Роялти", "Срок", "Оплачено", "Статус"]], use_container_width=True, hide_index=True)
                else:
                    st.info("История начислений пуста.")
        else:
            st.info("Выберите ваш офис в боковом меню.")
            
    # Recent billing notifications widget
    st.markdown("---")
    st.markdown("<p style='font-size:0.85rem; font-weight:700; margin-bottom:5px; color:#111; text-transform:uppercase;'>🔔 Лента уведомлений</p>", unsafe_allow_html=True)
    filtered_logs = []
    for log_entry in st.session_state.get("crm_telegram_logs", []):
        if any(keyword in log_entry for keyword in ["[PUSH]", "[СЧЕТ]", "[ОПЛАТА]", "[ТРЕВОГА]", "[АВТО-СИНХРОНИЗАЦИЯ]"]):
            clean_log = log_entry
            if log_entry.startswith("["):
                clean_log = log_entry[11:] 
            filtered_logs.append(clean_log)
            
    recent_alerts = filtered_logs[::-1][:3] 
    if recent_alerts:
        for alert in recent_alerts:
            icon = "🚨" if "[ТРЕВОГА]" in alert or "[ПРОСРОЧКА]" in alert else "🧾" if "[СЧЕТ]" in alert else "🟢" if "[ОПЛАТА]" in alert else "⚡" if "[АВТО-СИНХРОНИЗАЦИЯ]" in alert else "📱"
            st.markdown(f"""
            <div style="background-color: #FFFFFF; border: 1px solid #E2E2E6; padding: 10px; border-radius: 4px; margin-bottom: 8px; font-size: 0.75rem; line-height: 1.3;">
                <b>{icon} Оповещение:</b><br>{alert}
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown("<p style='font-size:0.75rem; color:#888;'>Лента пуста.</p>", unsafe_allow_html=True)
...
def show_corporate_footer():
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; color: #888888; font-size: 0.8rem; font-family: 'Barlow', sans-serif;">
        <div><b>©2026 CENTURY 21 Россия</b> • Корпоративная экосистема брокера v29.0 (Сквозной Биллинг & Финансовый комплаенс сети). CENTURY 21® зарегистрированный товарный знак (знак обслуживания), принадлежит компании Century 21 Real Estate LLC. [83]</div>
