...
<div style="padding: 12px; border-radius: 4px; background-color: #1A1A1A; border: 1px solid #2D2D2D; text-align: center;">
    <p style="font-size: 0.8rem; margin: 0; color: #C5A059; font-family: 'Barlow', sans-serif; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;"><b>Версия MVP: 18.0 (Премиальная Экосистема C21)</b></p>
...
# ==============================================================================
    ...
    with col_dc1:
        ...
        office_legal_type = st.selectbox("Организационно-правовая форма офиса:", ["ООО (Юридическое лицо)", "ИП (Индивидуальный предприниматель)"])
    ...
    with col_dc2:
        ...
        if is_min_tax_applied:
            st.warning("⚠️ **Внимание:** Расчетный налог 15% от операционной прибыли ({0:,.0f} ₽) меньше лимита. В соответствии со ст. 346.18 НК РФ, применен минимальный налог в размере **1% от валового дохода (ВКД)** сделки: **{1:,.0f} ₽** [18, 19].".format(max(0.0, operating_income * 0.15), deal_gci * 0.01))
        
        if ip_contributions > 0:
            st.info("👤 **Дополнительные взносы ИП (1%):** Поскольку прибыль по сделке превысила 300 000 ₽, дополнительный социальный взнос ИП по этой транзакции составит **{0:,.0f} ₽** (расчет по формуле: (Прибыль - 300 000) * 1% [20]).".format(ip_contributions))
    ...
    tab_8weeks, tab_13steps, tab_legal_tech = st.tabs([
...
def show_corporate_footer():
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; color: #888888; font-size: 0.8rem; font-family: 'Barlow', sans-serif;">
        <div><b>©2026 CENTURY 21 Россия</b> • Корпоративная экосистема брокера v18.0 (Премиальный MVP). CENTURY 21® зарегистрированный товарный знак (знак обслуживания), принадлежит компании Century 21 Real Estate LLC. [83]</div>
