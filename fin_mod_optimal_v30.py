...
if 'start_driver_other_vol' not in st.session_state:
    st.session_state['start_driver_other_vol'] = 0.0
...
# Инициализация расходов ЦО (Центрального Офиса)
if 'hq_royalty_deal_price' not in st.session_state:
    st.session_state['hq_royalty_deal_price'] = 2100.0
if 'hq_legal_services' not in st.session_state:
    st.session_state['hq_legal_services'] = 0.0
if 'hq_prof_insurance' not in st.session_state:
    st.session_state['hq_prof_insurance'] = 0.0
if 'hq_crm_price' not in st.session_state:
    st.session_state['hq_crm_price'] = 5500.0
if 'hq_cc_price' not in st.session_state:
    st.session_state['hq_cc_price'] = 14000.0
if 'hq_mortgage_commission_pct' not in st.session_state:
    st.session_state['hq_mortgage_commission_pct'] = 50.0
if 'hq_recruitment' not in st.session_state:
    st.session_state['hq_recruitment'] = 150000.0
if 'hq_overseas_commission_pct' not in st.session_state:
    st.session_state['hq_overseas_commission_pct'] = 30.0
...
with st.sidebar.expander("🏢 6. Выплаты в ЦО", expanded=False):
    ...
    st.session_state['hq_royalty_deal_price'] = royalty_deal_custom

    st.markdown("**Юр. сопровождение:**")
    hq_legal = st.number_input("Юр. сопровождение, ₽/месяц", min_value=0, max_value=500000,
                               value=int(st.session_state.get('hq_legal_services', 0.0)), step=5000)
    st.session_state['hq_legal_services'] = float(hq_legal)

    st.markdown("**Страхование проф. отв.:**")
    hq_insurance = st.number_input("Страхование проф. отв., ₽/месяц", min_value=0, max_value=500000,
                                   value=int(st.session_state.get('hq_prof_insurance', 0.0)), step=5000)
    st.session_state['hq_prof_insurance'] = float(hq_insurance)

    st.markdown("**CRM-система:**")
    hq_crm = st.number_input("CRM-система, ₽/месяц", min_value=0, max_value=50000,
                             value=int(st.session_state.get('hq_crm_price', 5500.0)), step=500)
    st.session_state['hq_crm_price'] = float(hq_crm)

    st.markdown("**Колл-центр (КЦ):**")
    hq_cc = st.number_input("Колл-центр (КЦ), ₽/месяц", min_value=0, max_value=100000,
                            value=int(st.session_state.get('hq_cc_price', 14000.0)), step=1000)
    st.session_state['hq_cc_price'] = float(hq_cc)

    st.markdown("**Ипотека: комиссия:**")
    hq_mort_pct = st.slider("Ипотека: комиссия, %", min_value=0, max_value=100,
                            value=int(st.session_state.get('hq_mortgage_commission_pct', 50.0)), step=5)
    st.session_state['hq_mortgage_commission_pct'] = float(hq_mort_pct)

    st.markdown("**Подбор персонала:**")
    hq_recr = st.number_input("Подбор персонала, ₽/месяц", min_value=0, max_value=500000,
                              value=int(st.session_state.get('hq_recruitment', 150000.0)), step=10000)
    st.session_state['hq_recruitment'] = float(hq_recr)

    st.markdown("**Зарубежная: комиссия:**")
    hq_overseas_pct = st.slider("Зарубежная: комиссия, %", min_value=0, max_value=100,
                                value=int(st.session_state.get('hq_overseas_commission_pct', 30.0)), step=5)
    st.session_state['hq_overseas_commission_pct'] = float(hq_overseas_pct)
...
hq_legal_services_list = []
hq_prof_insurance_list = []
hq_crm_list = []
hq_cc_list = []
hq_mortgage_commission_list = []
hq_recruitment_list = []
hq_overseas_commission_list = []
hq_kc_list = []
...
for i in range(24):
    ...
    default_deals_other_vol = float(st.session_state.get('start_driver_other_vol', 0.0))
    ...
    # Define counts for backoffice roles based on headcount overrides and standard defaults
    rop_C_count = get_headcount_override('  ├─ РОП: категория C', str(i+1), DEFAULT_HEADCOUNTS['РОП: категория C'][i])
    if (i+1) >= 3 and st.session_state.get('add_rop_m3', False):
        rop_C_count += 1

    rop_B_count = get_headcount_override('  ├─ РОП: категория B', str(i+1), DEFAULT_HEADCOUNTS['РОП: категория B'][i])

    admin_count = get_headcount_override('  ├─ Администратор офиса', str(i+1), DEFAULT_HEADCOUNTS['Администратор офиса'][i])
    if (i+1) >= 6 and st.session_state.get('add_admin_m6', False):
        admin_count += 1

    hr_count = get_headcount_override('  ├─ HR/рекрутер', str(i+1), DEFAULT_HEADCOUNTS['HR/рекрутер'][i])
    ros_count = get_headcount_override('  ├─ РОС/тренер', str(i+1), DEFAULT_HEADCOUNTS['РОС/тренер'][i])
    jurist_count = get_headcount_override('  ├─ Юрист', str(i+1), DEFAULT_HEADCOUNTS['Юрист'][i])
    mort_broker_count = get_headcount_override('  ├─ Ипотечный Брокер', str(i+1), DEFAULT_HEADCOUNTS['Ипотечный Брокер'][i])
    listing_count = get_headcount_override('  ├─ Листинг-менеджер', str(i+1), DEFAULT_HEADCOUNTS['Листинг-менеджер'][i])
    photographer_count = get_headcount_override('  ├─ Фотограф', str(i+1), DEFAULT_HEADCOUNTS['Фотограф'][i])
    smm_count = get_headcount_override('  ├─ Маркетолог/SMM', str(i+1), DEFAULT_HEADCOUNTS['Маркетолог/SMM'][i])
    ...
    # 6. ВЫПЛАТЫ ЦО (Франшиза и расходы Центрального Офиса)
    hq_fix = get_override('  ├─ Роялти FIX (вкл. НРФ)', str(i+1), base['hq_royalty_fix'][i] * royalty_fix_mult)
    hq_royalty_fix_list.append(hq_fix)

    # Роялти за сделку: комиссия по количеству сделок
    royalty_deal_price = st.session_state.get('hq_royalty_deal_price', 2100.0)
    hq_deal = get_override('  ├─ Роялти за сделку', str(i+1), (deals_sec_vol + deals_prim_vol + deals_sub_vol + deals_rent_vol) * royalty_deal_price)
    hq_royalty_deal_list.append(hq_deal)

    # Юр. сопровождение
    hq_legal = get_override('  ├─ Юр. сопровождение', str(i+1), st.session_state.get('hq_legal_services', 0.0))
    hq_legal_services_list.append(hq_legal)

    # Страхование проф. отв.
    hq_insurance = get_override('  ├─ Страхование проф. отв.', str(i+1), st.session_state.get('hq_prof_insurance', 0.0))
    hq_prof_insurance_list.append(hq_insurance)

    # CRM-система
    hq_crm = get_override('  ├─ CRM-система', str(i+1), st.session_state.get('hq_crm_price', 5500.0))
    hq_crm_list.append(hq_crm)

    # Колл-центр
    hq_cc = get_override('  ├─ Колл-центр (КЦ)', str(i+1), st.session_state.get('hq_cc_price', 14000.0))
    hq_cc_list.append(hq_cc)

    # Ипотека: комиссия (процент от сделок ипотеки)
    mortgage_commission_pct = st.session_state.get('hq_mortgage_commission_pct', 50.0)
    hq_mortgage_comm = get_override('  ├─ Ипотека: комиссия', str(i+1), serv_mort_vol * (mortgage_commission_pct / 100.0))
    hq_mortgage_commission_list.append(hq_mortgage_comm)

    # Подбор персонала
    hq_recr = get_override('  ├─ Подбор персонала', str(i+1), st.session_state.get('hq_recruitment', 150000.0))
    hq_recruitment_list.append(hq_recr)

    # Зарубежная: комиссия (процент от зарубежных сделок)
    overseas_commission_pct = st.session_state.get('hq_overseas_commission_pct', 30.0)
    hq_overseas_comm = get_override('  ├─ Зарубежная: комиссия', str(i+1), deals_overseas_vol * (overseas_commission_pct / 100.0))
    hq_overseas_commission_list.append(hq_overseas_comm)

    # Прочие сервисы КЦ и ЦО
    hq_kc_val = get_override('  └─ Прочие сервисы ЦО', str(i+1), max(0.0, base['hq_crm_and_others'][i] - hq_crm))
    hq_kc_list.append(hq_kc_val)

    total_hq_payments = hq_fix + hq_deal + hq_legal + hq_insurance + hq_crm + hq_cc + hq_mortgage_comm + hq_recr + hq_overseas_comm + hq_kc_val
...
v_m0_hq_deal = get_override('  ├─ Роялти за сделку', 'Старт', 0.0)
v_m0_hq_legal = get_override('  ├─ Юр. сопровождение', 'Старт', 0.0)
v_m0_hq_insurance = get_override('  ├─ Страхование проф. отв.', 'Старт', 0.0)
v_m0_hq_crm = get_override('  ├─ CRM-система', 'Старт', 0.0)
v_m0_hq_cc = get_override('  ├─ Колл-центр (КЦ)', 'Старт', 0.0)
v_m0_hq_mortgage = get_override('  ├─ Ипотека: комиссия', 'Старт', 0.0)
v_m0_hq_recruitment = get_override('  ├─ Подбор персонала', 'Старт', 0.0)
v_m0_hq_overseas = get_override('  ├─ Зарубежная: комиссия', 'Старт', 0.0)
v_m0_hq_kc = get_override('  └─ Прочие сервисы ЦО', 'Старт', 0.0)
v_m0_hq_total = v_m0_hq_fix + v_m0_hq_deal + v_m0_hq_legal + v_m0_hq_insurance + v_m0_hq_crm + v_m0_hq_cc + v_m0_hq_mortgage + v_m0_hq_recruitment + v_m0_hq_overseas + v_m0_hq_kc
...
hq_rows_def = [
    ...
    ('Роялти за сделку', hq_royalty_deal_list, v_m0_hq_deal, '  ├─ Роялти за сделку'),
    ('Юр. сопровождение', hq_legal_services_list, v_m0_hq_legal, '  ├─ Юр. сопровождение'),
    ('Страхование проф. отв.', hq_prof_insurance_list, v_m0_hq_insurance, '  ├─ Страхование проф. отв.'),
    ('CRM-система', hq_crm_list, v_m0_hq_crm, '  ├─ CRM-система'),
    ('Колл-центр (КЦ)', hq_cc_list, v_m0_hq_cc, '  ├─ Колл-центр (КЦ)'),
    ('Ипотека: комиссия', hq_mortgage_commission_list, v_m0_hq_mortgage, '  ├─ Ипотека: комиссия'),
    ('Подбор персонала', hq_recruitment_list, v_m0_hq_recruitment, '  ├─ Подбор персонала'),
    ('Зарубежная: комиссия', hq_overseas_commission_list, v_m0_hq_overseas, '  ├─ Зарубежная: комиссия'),
    ('Прочие сервисы ЦО', hq_kc_list, v_m0_hq_kc, '  └─ Прочие сервисы ЦО'),
...
# Селектор режима работы — крупные карточки-вкладки (переменные для описаний)
_mode_desc = {
    "🎯 Презентация и Шаговый конструктор запуска (Sales)":
        "Линейный квест из 5 шагов для презентации франшизы потенциальному партнеру",
    "📐 Детальный Интерактивный Калькулятор (Calculator)":
        "Финансовая лаборатория: настройка каждой переменной модели «до рубля»",
    "🛠️ Сопровождение и Диагностика офиса (Success)":
        "Аудит действующего офиса: светофор отклонений и рецепты роста",
}
_mode_colors = {
    "🎯 Презентация и Шаговый конструктор запуска (Sales)": "#BEAF87",
    "📐 Детальный Интерактивный Калькулятор (Calculator)": "#0f766e",
    "🛠️ Сопровождение и Диагностика офиса (Success)": "#b21d1d",
}

# CSS для крупных карточек-режимов поверх стандартного radio
st.markdown("""
<style>
/* Режимные вкладки: прячем кружки radio, превращаем строки в карточки */
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] {
    gap: 0.6rem;
}
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label {
    background: linear-gradient(135deg, #fdfcfa 0%, #f6f3ec 100%);
...
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label:hover {
    border-color: #BEAF87;
    box-shadow: 0 6px 18px rgba(190,175,135,0.28);
    transform: translateY(-2px);
}
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label[data-checked="true"],
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label:has(input:checked) {
    background: linear-gradient(135deg, #f4eddb 0%, #fdfcfa 100%);
    border: 2px solid #BEAF87;
    box-shadow: 0 6px 20px rgba(190,175,135,0.35), inset 0 0 0 1px #BEAF87;
}
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label[data-checked="true"] div:nth-child(2) p,
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label:has(input:checked) div:nth-child(2) p {
    font-weight: 800;
    color: #1e1f22;
}
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label:has(input:not(:checked)) div:nth-child(2) p {
    font-weight: 600;
    color: #5c5f66;
}
/* Прячем стандартный кружок radio */
div[role="radiogroup"][aria-label="Выберите режим работы приложения:"] label > div:first-child {
    display: none;
}
</style>
""", unsafe_allow_html=True)
...
# Описание выбранного режима под вкладками
st.markdown(
    f"<div style='margin-top:-8px; padding: 0.55rem 1rem; border-left: 4px solid {_mode_colors[app_mode]}; "
    f"background: #f8f7f3; border-radius: 0 10px 10px 0; font-size: 0.88rem; color: #5c5f66;'>"
    f"{_mode_desc[app_mode]}</div>",
    unsafe_allow_html=True
)
...
if app_mode == "🎯 Презентация и Шаговый конструктор запуска (Sales)":
    ...
    st.markdown("ℹ️ *Этот режим предоставляет полный контроль над финансовым моделированием. Вы можете детально настроить CAPEX, кадровый план, доходы, расходы и мгновенно увидеть результат в виде интерактивного P&L-отчета.*")

    calc_tab_capex, calc_tab_staff, calc_tab_revenue, calc_tab_expenses, calc_tab_pl, calc_tab_management, calc_tab_reference = st.tabs([
        "📐 1. Калькулятор CAPEX",
        "👥 2. Кадровое планирование (чел.)",
        "📈 3. Доходы (Воронка продаж & ВКД)",
        "💰 4. Расходы (OPEX, ФОТ, ЦО)",
        "📋 5. Финансовый отчет P&L (₽)",
        "⚙️ 6. Управление данными",
        "📝 7. Справочник и Методология"
    ])
    ...
    with calc_tab_capex:
        ...
        with col_calc_right:
            st.info("💡 Здесь будет отображаться структура стартового бюджета после настройки параметров слева.")

    with calc_tab_expenses:
    ...
    with calc_tab_pl:
        st.markdown("""
        <style>
        .pl-shell { background: #f8f7f3; border-radius: 18px; padding: 1rem; border: 1px solid #eadfb8; }
        .pl-kpi-card {
        ...
        .pl-kpi-label { color: #5c5f66; font-size: 0.76rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; }
        .pl-kpi-value { font-size: 1.55rem; font-weight: 800; color: #1e1f22; margin-top: 0.4rem; line-height: 1.2; }
        .pl-kpi-sub { font-size: 0.76rem; color: #6b7280; margin-top: 0.3rem; }
        .pl-green { color: #0f766e; }
        .pl-red { color: #b21d1d; }
        .pl-gold { color: #8b6d2d; }
        div[data-testid="stExpander"] > div:first-child { background: #f9f9f6; border: 1px solid #e7e0ca; border-radius: 12px; }
        div[data-testid="stDataFrame"] { border-radius: 12px; overflow: hidden; }
        </style>
        """, unsafe_allow_html=True)

        # ── Шапка отчета: заголовок + переключатель масштаба в одну строку ──
        st.markdown("#### 📋 Сводный финансовый дашборд P&L (₽)")
        head_l, head_r = st.columns([2.2, 1.3])
        with head_l:
            st.markdown("ℹ️ *Колонка «Старт» (Месяц 0) не учитывается в KPI и сводке — только фактические периоды.*")
        with head_r:
        ...
        def period_series(values_list, start_value):
            arr = aggregate_metric(values_list, start_value, calc_time_scale)
            return arr[1:] if len(arr) > 1 else []

        def color_positive_negative(value):
            try:
                num = float(str(value).replace(' ', '').replace('₽', '').replace(',', '.').replace('%', ''))
            except Exception:
                return 'color: #1f2937; font-weight: 600;'
            if num >= 0:
                return 'color: #0f766e; font-weight: 700; background: rgba(15,118,110,0.08);'
            return 'color: #b21d1d; font-weight: 700; background: rgba(178,29,29,0.08);'

        def make_dashboard_table(rows_def):
            if not rows_def:
                return pd.DataFrame({'Показатель': []})
            period_count = len(aggregate_metric(rows_def[0][1], rows_def[0][2], calc_time_scale))
            records = []
            for row in rows_def:
                values = aggregate_metric(row[1], row[2], calc_time_scale)
                rec = {'Показатель': row[0]}
                for i in range(1, period_count):
                    rec[f'P{i}'] = float(values[i])
                records.append(rec)
            df = pd.DataFrame(records)
            return df

        revenue_series = period_series(rev_total_list, v_m0_rev_total)
        agent_series = period_series(payouts_agents_list, v_m0_agent_total)
        backoffice_series = period_series(salaries_backoffice_list, v_m0_back_total)
        opex_series = period_series(total_opex_list, v_m0_opex_total)
        hq_series = period_series(total_hq_payments_list, v_m0_hq_total)
        total_expense_series = [a + b + c + d for a, b, c, d in zip(agent_series, backoffice_series, opex_series, hq_series)]
        capex_series = period_series(total_capex_list, total_capex_m0)
        net_series = [r - e for r, e in zip(revenue_series, total_expense_series)]

        current_revenue = revenue_series[-1] if revenue_series else 0.0
        current_expense = total_expense_series[-1] if total_expense_series else 0.0
        current_capex = capex_series[-1] if capex_series else 0.0
        current_net = net_series[-1] if net_series else 0.0

        # ═══ СЕКЦИЯ 1: ИТОГИ ПРОЕКТА (самое главное — первым) ═══
        st.markdown("#### 🏆 Итоги проекта (2 года)")
        rev_txt = f"{total_revenue_2years:,.0f} ₽".replace(",", " ")
        prof_txt = f"{total_profit_2years:,.0f} ₽".replace(",", " ")
        maxinv_txt = f"{max_investment:,.0f} ₽".replace(",", " ")
        proj_cols = st.columns(5)
        with proj_cols[0]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>💼 Общая выручка</div><div class='pl-kpi-value pl-green'>{rev_txt}</div><div class='pl-kpi-sub'>За 2 года проекта</div></div>", unsafe_allow_html=True)
        with proj_cols[1]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>📈 Чистая прибыль</div><div class='pl-kpi-value pl-green'>{prof_txt}</div><div class='pl-kpi-sub'>Итог за 2 года</div></div>", unsafe_allow_html=True)
        with proj_cols[2]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>📉 Пик инвестиций</div><div class='pl-kpi-value pl-red'>{maxinv_txt}</div><div class='pl-kpi-sub'>CAPEX + OPEX</div></div>", unsafe_allow_html=True)
        with proj_cols[3]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>🎯 Точка безубыточности</div><div class='pl-kpi-value pl-gold'>{breakeven_month}</div><div class='pl-kpi-sub'>Первый прибыльный мес.</div></div>", unsafe_allow_html=True)
        with proj_cols[4]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>⏳ Окупаемость</div><div class='pl-kpi-value pl-gold'>{payback_month}</div><div class='pl-kpi-sub'>Полный возврат вложений</div></div>", unsafe_allow_html=True)

        st.markdown("<div style='height: 6px'></div>", unsafe_allow_html=True)

        # ═══ СЕКЦИЯ 2: KPI ТЕКУЩЕГО ПЕРИОДА ═══
        kpi_cols = st.columns(4)
        with kpi_cols[0]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>Доходы</div><div class='pl-kpi-value pl-green'>{fmt(current_revenue)}</div><div class='pl-kpi-sub'>Итог последнего периода</div></div>", unsafe_allow_html=True)
        with kpi_cols[1]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>Расходы</div><div class='pl-kpi-value pl-red'>{fmt(current_expense)}</div><div class='pl-kpi-sub'>ФОТ + OPEX + ЦО</div></div>", unsafe_allow_html=True)
        with kpi_cols[2]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>🛠️ Инвестиции (CAPEX)</div><div class='pl-kpi-value pl-gold'>{fmt(current_capex)}</div><div class='pl-kpi-sub'>Капитальные вложения</div></div>", unsafe_allow_html=True)
        with kpi_cols[3]:
            st.markdown(f"<div class='pl-kpi-card'><div class='pl-kpi-label'>Результат</div><div class='pl-kpi-value'>{fmt(current_net)}</div><div class='pl-kpi-sub'>Чистый результат</div></div>", unsafe_allow_html=True)

        def style_numeric_columns(df, total_row=None):
            def colorize(value):
                try:
                    num = float(value)
            ...
            styled = df.style.apply(
                lambda col: [
                    colorize(v) if col.name != 'Показатель' else ''
                    for v in col
                ],
                axis=0
            )
            # Форматируем числовые колонки: разрядные пробелы, без дробей
            num_cols = [c for c in df.columns if c != 'Показатель']
            styled = styled.format({c: lambda v: f"{v:,.0f}".replace(",", " ") for c in num_cols})
            # Подсветка итоговой строки
            def highlight_total(row):
                if total_row and row['Показатель'] == total_row:
                    return ['background: rgba(190,175,135,0.25); font-weight: 800;'] * len(row)
                return [''] * len(row)
            styled = styled.apply(highlight_total, axis=1)
            return styled

        # ═══ СЕКЦИЯ 3: ДИНАМИКА (графики в две колонки) ═══
        st.markdown("#### 📊 Динамика по периодам")

        chart_l, chart_r = st.columns([1.4, 1])
        with chart_l:
            # Доходы vs расходы vs чистый результат
            periods_count = max(len(revenue_series), len(total_expense_series))
            fig_pl = go.Figure()
            fig_pl.add_trace(go.Bar(
                name='Доходы',
                x=list(range(1, periods_count + 1)),
                y=revenue_series,
                marker_color='#0f766e',
                hovertemplate='Доходы: %{y:,.0f} ₽<extra></extra>'
            ))
            fig_pl.add_trace(go.Bar(
                name='Расходы',
                x=list(range(1, periods_count + 1)),
                y=total_expense_series,
                marker_color='#b21d1d',
                hovertemplate='Расходы: %{y:,.0f} ₽<extra></extra>'
            ))
            fig_pl.add_trace(go.Scatter(
                name='Чистый результат',
                mode='lines+markers',
                x=list(range(1, periods_count + 1)),
                y=net_series,
                line=dict(color='#8b6d2d', width=3),
                hovertemplate='Чистый: %{y:,.0f} ₽<extra></extra>'
            ))
            fig_pl.update_layout(
                barmode='group',
                title='Доходы / Расходы / Чистый результат',
                xaxis_title='Период',
                yaxis_title='₽',
                template='plotly_white',
                height=360,
        ...
        with chart_r:
            # Структура расходов за весь проект (пончик)
            exp_totals = {
                'ФОТ: агенты': sum(payouts_agents_list) + v_m0_agent_total,
                'ФОТ: оклады': sum(salaries_backoffice_list) + v_m0_back_total,
                'OPEX': sum(total_opex_list) + v_m0_opex_total,
                'Выплаты ЦО': sum(total_hq_payments_list) + v_m0_hq_total,
                'CAPEX': sum(total_capex_list) + total_capex_m0,
            }
            fig_donut = go.Figure(go.Pie(
                labels=list(exp_totals.keys()),
                values=list(exp_totals.values()),
                hole=0.55,
                marker=dict(colors=['#8b6d2d', '#a19276', '#b21d1d', '#c9a86a', '#5c5f66']),
                textinfo='percent',
                hovertemplate='%{label}: %{value:,.0f} ₽ (%{percent})<extra></extra>'
            ))
            fig_donut.update_layout(
                title='Структура расходов за проект',
                template='plotly_white',
                height=340,
        ...
        # ═══ СЕКЦИЯ 4: СВОДНАЯ ТАБЛИЦА ═══
        st.markdown("#### 📅 Финансовая сводка по периодам")
        summary_df = pd.DataFrame({
            'Показатель': ['Доходы', 'Расходы', 'Инвестиции (CAPEX)', 'Чистый результат'],
            **{f'P{i+1}': [
                revenue_series[i] if i < len(revenue_series) else 0.0,
                total_expense_series[i] if i < len(total_expense_series) else 0.0,
                capex_series[i] if i < len(capex_series) else 0.0,
                net_series[i] if i < len(net_series) else 0.0,
            ] for i in range(max(len(revenue_series), len(total_expense_series), len(capex_series), len(net_series)))}
        })

        st.dataframe(style_numeric_columns(summary_df), use_container_width=True, hide_index=True)

        # ═══ СЕКЦИЯ 5: ДЕТАЛИЗАЦИЯ ПО РАЗДЕЛАМ ═══
        expense_categories = [
            ('ФОТ: агентское вознаграждение', payouts_agents_list, v_m0_agent_total),
            ('ФОТ: оклады', salaries_backoffice_list, v_m0_back_total),
            ('OPEX', total_opex_list, v_m0_opex_total),
            ('Центральный Офис', total_hq_payments_list, v_m0_hq_total),
        ]
        st.markdown("#### 🔍 Детализация по разделам")
        st.markdown("ℹ️ *Раскройте нужный раздел, чтобы увидеть постатейную разбивку по всем периодам.*")

        # Доходы — отдельный блок (источник прибыли)
        with st.expander("🟢 Доходы (ВКД)", expanded=False):
            revenue_df = make_dashboard_table(revenue_rows_def)
            if not revenue_df.empty:
                st.dataframe(style_numeric_columns(revenue_df), use_container_width=True, hide_index=True)

        # Все расходы — визуально сгруппированы под подзаголовком
        st.markdown("**Расходные разделы:**")

        with st.expander("👥 ФОТ: Расходы на персонал", expanded=False):
            fot_df = make_dashboard_table(fot_rows_def)
            if not fot_df.empty:
                st.dataframe(style_numeric_columns(fot_df), use_container_width=True, hide_index=True)

        with st.expander("🏢 Операционные расходы (OPEX)", expanded=False):
            opex_df = make_dashboard_table(opex_rows_def)
            if not opex_df.empty:
                st.dataframe(style_numeric_columns(opex_df), use_container_width=True, hide_index=True)

        with st.expander("👑 Выплаты в ЦО", expanded=False):
            hq_df = make_dashboard_table(hq_rows_def)
            if not hq_df.empty:
                st.dataframe(style_numeric_columns(hq_df), use_container_width=True, hide_index=True)

        with st.expander("💰 Расходы Итого (по категориям)", expanded=False):
            expense_rows = []
            for label, values, start in expense_categories:
                series = period_series(values, start)
                expense_rows.append({'Показатель': label, **{f'P{i+1}': v for i, v in enumerate(series)}})
            total_series = [sum(vals[i] for vals in [agent_series, backoffice_series, opex_series, hq_series]) for i in range(max(len(agent_series), len(backoffice_series), len(opex_series), len(hq_series)))]
            expense_rows.append({'Показатель': '★ ИТОГО РАСХОДЫ', **{f'P{i+1}': total_series[i] for i in range(len(total_series))}})
            expense_df = pd.DataFrame(expense_rows)
            st.dataframe(style_numeric_columns(expense_df, total_row='★ ИТОГО РАСХОДЫ'), use_container_width=True, hide_index=True)

        # Инвестиции — финальный блок
        with st.expander("🛠️ Капитальные вложения (CAPEX, Месяц 0)", expanded=False):
            capex_df = make_dashboard_table(capex_rows_def)
            if not capex_df.empty:
                st.dataframe(style_numeric_columns(capex_df), use_container_width=True, hide_index=True)


    with calc_tab_revenue:
        st.markdown("#### 📈 Операционные показатели и Драйверы воронки (шт.)")
        st.markdown("ℹ️ *Здесь настраиваются плановые показатели воронки продаж и конверсии (в штуках) по месяцам 1-24. В колонке **Старт** вводятся базовые параметры: % конверсии, количество мест и т.п. для других показателей; для МЛС — абсолютное количество сделок. Все значения можно редактировать вручную в месячных ячейках, и вся цепочка мгновенно пересчитается.*")
        ...
        df_drivers_data['Старт'] = [
            ...
            float(st.session_state.get('start_driver_other_vol', 0.0)),
        ...
        for idx, col in enumerate(months):
            ...
            # МЛС теперь считаются абсолютным количеством сделок, а не процентом
            ...
            d_other = get_driver_override('  ├─ Сделки: прочее (МЛС, срочновыкуп, сайт)', col, float(st.session_state.get('start_driver_other_vol', 0.0)))
        ...
        for idx_row in range(len(df_drivers)):
            ...
            if float(val_base_start) != float(val_edited_start):
                ...
                elif idx_row == 9: st.session_state['start_driver_other_vol'] = parsed_val
        ...
        <div style='background-color: #F8F9FA; padding: 20px; border-radius: 8px; border-left: 5px solid #BEAF87;'>
            ...
            <ul style='font-size: 12.5px; color: #555; padding-left: 20px; line-height: 1.6;'>
                ...
                <li>🔄 <b>Сделки: прочее (МЛС, срочновыкуп, сайт) в единицах:</b> Количество сделок рассчитывается на основе значения, которое вы установите в колонке <code>Старт</code> (абсолютное количество сделок). Для каждого месяца можно переопределить это значение вручную в ячейках таблицы.</li>
    ...
    with calc_tab_management:
        st.markdown("#### ⚙️ Управление данными расходов и доходов")
        st.markdown("📋 *Интерактивный редактор всех параметров модели. Здесь вы можете просмотреть, отредактировать и загрузить все значения параметров.*")

        # Создаем DataFrame со всеми параметрами
        params_data = {
            'Раздел': [
                # ДОХОДЫ
                'ДОХОДЫ', 'ДОХОДЫ', 'ДОХОДЫ', 'ДОХОДЫ', 'ДОХОДЫ', 'ДОХОДЫ',
                # ФОТ Агенты
                'ФОТ (Агенты)', 'ФОТ (Агенты)', 'ФОТ (Агенты)', 'ФОТ (Агенты)',
                # Бэк-офис оклады
                'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)', 'ФОТ (Оклады)',
                # OPEX
                'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX', 'OPEX',
                # Расходы ЦО
                'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО', 'Расходы ЦО',
                # CAPEX
                'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX', 'CAPEX'
            ],
            'Параметр': [
                # ДОХОДЫ
                'Вторичный рынок (₽)', 'Первичный рынок (₽)', 'Аренда (₽)', 'Загородная (₽)', 'Зарубежная (₽)', 'Прочее МЛС (₽)',
                # ФОТ Агенты
                'Агент D (%)', 'Агент C (%)', 'Агент B (%)', 'Агент A (%)',
                # Оклады
                'РОП C (₽)', 'РОП B (₽)', 'Администратор (₽)', 'HR/рекрутер (₽)', 'РОС/тренер (₽)', 'Юрист (₽)', 'Ипотечный Брокер (₽)', 'Листинг (₽)', 'Фотограф (₽)', 'SMM (₽)',
                # OPEX
                'Интернет (₽)', 'Сотовая связь (₽)', 'Канцелярия (₽)', 'Реклама объектов (₽)', 'HeadHunter (₽)', 'Бухгалтерия (₽)', 'Услуги банка (₽)', 'Аренда за м² (₽)', 'Уборка (₽)', 'ГСМ (₽)', 'Курьер (₽)', 'Корпоративы (₽)',
                # Расходы ЦО
                'Роялти за сделку (₽)', 'Юр. сопровождение (₽)', 'Страхование проф (₽)', 'CRM-система (₽)', 'Колл-центр (₽)', 'Ипотека комиссия (%)', 'Подбор персонала (₽)', 'Зарубежная комиссия (%)', 'РОП C бонус (%)', 'РОП B бонус (%)',
                # CAPEX
                'Паушальный взнос (₽)', 'Роспатент (₽)', 'Площадь офиса (м²)', 'Норма м² на чел', 'Рабочие места (шт)', 'Стол (₽)', 'Стул (₽)', 'ПК (₽)', 'Доп. мебель (₽)'
            ],
            'Значение': [
                # ДОХОДЫ
                float(st.session_state.get('comm_secondary', 360000.0)),
                ...
                float(st.session_state.get('comm_other_p', 252000.0)),
                # ФОТ Агенты
                float(st.session_state.get('rate_agent_D', 35.0)),
                float(st.session_state.get('rate_agent_C', 40.0)),
                float(st.session_state.get('rate_agent_B', 45.0)),
                float(st.session_state.get('rate_agent_A', 50.0)),
                # Оклады
                float(st.session_state.get('sal_rop_C', 70000.0)),
                float(st.session_state.get('sal_rop_B', 100000.0)),
                float(st.session_state.get('sal_admin', 70000.0)),
                float(st.session_state.get('sal_hr', 100000.0)),
                float(st.session_state.get('sal_ros', 100000.0)),
                float(st.session_state.get('sal_jurist', 150000.0)),
                float(st.session_state.get('sal_mort', 180000.0)),
                float(st.session_state.get('sal_listing', 50000.0)),
                float(st.session_state.get('sal_photo', 30000.0)),
                float(st.session_state.get('sal_smm', 150000.0)),
                # OPEX
                float(st.session_state.get('opex_internet', 5000.0)),
                float(st.session_state.get('opex_mobile', 600.0)),
                float(st.session_state.get('opex_kanc', 500.0)),
                float(st.session_state.get('opex_reklama', 7000.0)),
                float(st.session_state.get('opex_hh', 45000.0)),
                float(st.session_state.get('opex_buh', 20000.0)),
                float(st.session_state.get('opex_bank', 2000.0)),
                float(st.session_state.get('opex_rent_price_m2', 2500.0)),
                float(st.session_state.get('opex_cleaning', 15000.0)),
                float(st.session_state.get('opex_gsm', 5000.0)),
                float(st.session_state.get('opex_courier', 10000.0)),
                float(st.session_state.get('opex_events', 10000.0)),
                # Расходы ЦО
                float(st.session_state.get('hq_royalty_deal_price', 2100.0)),
                float(st.session_state.get('hq_legal_services', 0.0)),
                float(st.session_state.get('hq_prof_insurance', 0.0)),
                float(st.session_state.get('hq_crm_price', 5500.0)),
                float(st.session_state.get('hq_cc_price', 14000.0)),
                float(st.session_state.get('hq_mortgage_commission_pct', 50.0)),
                float(st.session_state.get('hq_recruitment', 150000.0)),
                float(st.session_state.get('hq_overseas_commission_pct', 30.0)),
                float(st.session_state.get('rate_rop_C_bonus', 3.0)),
                float(st.session_state.get('rate_rop_B_bonus', 12.0)),
                # CAPEX
                float(st.session_state.get('capex_pau_fee', 577500.0)),
                float(st.session_state.get('capex_rospatent', 17000.0)),
                float(st.session_state.get('capex_office_area', 60.0)),
                float(st.session_state.get('capex_norm_m2', 6.0)),
                float(st.session_state.get('capex_workstations', 10)),
                float(st.session_state.get('capex_table_price', 12000.0)),
                float(st.session_state.get('capex_stool_price', 7000.0)),
                float(st.session_state.get('capex_computer_price', 45000.0)),
                float(st.session_state.get('capex_add_furniture', 150000.0))
            ]
        }

        df_params = pd.DataFrame(params_data)

        st.subheader("📊 Таблица всех параметров")
        edited_df = st.data_editor(df_params, use_container_width=True, num_rows="fixed", key="params_editor")

        # Кнопка сохранения
        col_save, col_export, col_import = st.columns(3)
        with col_save:
            if st.button("💾 Применить все изменения", key="save_params"):
                # Маппинг параметров к session_state ключам
                param_mapping = {
                    'Вторичный рынок (₽)': 'comm_secondary',
                    'Первичный рынок (₽)': 'comm_primary',
                    'Аренда (₽)': 'comm_rent_val',
                    'Загородная (₽)': 'comm_suburban',
                    'Зарубежная (₽)': 'comm_overseas',
                    'Прочее МЛС (₽)': 'comm_other_p',
                    'Агент D (%)': 'rate_agent_D',
                    'Агент C (%)': 'rate_agent_C',
                    'Агент B (%)': 'rate_agent_B',
                    'Агент A (%)': 'rate_agent_A',
                    'РОП C (₽)': 'sal_rop_C',
                    'РОП B (₽)': 'sal_rop_B',
                    'Администратор (₽)': 'sal_admin',
                    'HR/рекрутер (₽)': 'sal_hr',
                    'РОС/тренер (₽)': 'sal_ros',
                    'Юрист (₽)': 'sal_jurist',
                    'Ипотечный Брокер (₽)': 'sal_mort',
                    'Листинг (₽)': 'sal_listing',
                    'Фотограф (₽)': 'sal_photo',
                    'SMM (₽)': 'sal_smm',
                    'Интернет (₽)': 'opex_internet',
                    'Сотовая связь (₽)': 'opex_mobile',
                    'Канцелярия (₽)': 'opex_kanc',
                    'Реклама объектов (₽)': 'opex_reklama',
                    'HeadHunter (₽)': 'opex_hh',
                    'Бухгалтерия (₽)': 'opex_buh',
                    'Услуги банка (₽)': 'opex_bank',
                    'Аренда за м² (₽)': 'opex_rent_price_m2',
                    'Уборка (₽)': 'opex_cleaning',
                    'ГСМ (₽)': 'opex_gsm',
                    'Курьер (₽)': 'opex_courier',
                    'Корпоративы (₽)': 'opex_events',
                    'Роялти за сделку (₽)': 'hq_royalty_deal_price',
                    'Юр. сопровождение (₽)': 'hq_legal_services',
                    'Страхование проф (₽)': 'hq_prof_insurance',
                    'CRM-система (₽)': 'hq_crm_price',
                    'Колл-центр (₽)': 'hq_cc_price',
                    'Ипотека комиссия (%)': 'hq_mortgage_commission_pct',
                    'Подбор персонала (₽)': 'hq_recruitment',
                    'Зарубежная комиссия (%)': 'hq_overseas_commission_pct',
                    'РОП C бонус (%)': 'rate_rop_C_bonus',
                    'РОП B бонус (%)': 'rate_rop_B_bonus',
                    'Паушальный взнос (₽)': 'capex_pau_fee',
                    'Роспатент (₽)': 'capex_rospatent',
                    'Площадь офиса (м²)': 'capex_office_area',
                    'Норма м² на чел': 'capex_norm_m2',
                    'Рабочие места (шт)': 'capex_workstations',
                    'Стол (₽)': 'capex_table_price',
                    'Стул (₽)': 'capex_stool_price',
                    'ПК (₽)': 'capex_computer_price',
                    'Доп. мебель (₽)': 'capex_add_furniture'
                }

                # Обновляем session_state из отредактированной таблицы
                for idx, row in edited_df.iterrows():
                    param = row['Параметр']
                    value = row['Значение']
                    if param in param_mapping:
                        st.session_state[param_mapping[param]] = float(value)

                st.success("✅ Все параметры обновлены! Модель пересчитается автоматически.")
                st.rerun()
        ...
        with col_import:
            st.info("💡 Совет: Редактируйте таблицу выше и нажмите 'Применить' для сохранения.")

    with calc_tab_reference:
        st.markdown("""
