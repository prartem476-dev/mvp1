...
# Селектор режима работы на самом верху экрана (Sales vs Success)
app_mode = st.radio(
    "⚡ **Выберите режим работы приложения:**",
    options=["🎯 Презентация и Шаговый конструктор запуска (Sales)", "🛠️ Сопровождение и Диагностика офиса (Success)"],
    horizontal=True,
...
if app_mode == "🎯 Презентация и Шаговый конструктор запуска (Sales)":
    ...
    if step == "1. Локация и Масштаб":
        ...
        col_left, col_right = st.columns([2, 1.2])
        with col_left:
            st.markdown("#### 🛠️ Шаг 2: Капитальные вложения (CAPEX)")
            st.write("Сконструируйте ваш стартовый пакет офиса. Выберите уровень ремонта и количество рабочих мест:")
            
            # Красивые Кнопки-Переключатели
            col_b1, col_fill_temp, col_b2 = st.columns([1, 0.1, 1])
            with col_b1:
                st.markdown("**🔧 Категория Ремонта и Брендинга:**")
                renov_pkg = st.radio(
                    "Выберите пакет ремонта:",
                    options=["Стандарт C21 (520 000 ₽)", "Премиум-дизайн (1 000 000 ₽)", "Свой бюджет (ввести вручную)"],
                    key="renov_radio_pkg"
                )
                if renov_pkg == "Стандарт C21 (520 000 ₽)":
                    renov_val = 520000
                elif renov_pkg == "Премиум-дизайн (1 000 000 ₽)":
                    renov_val = 1000000
                else:
                    renov_val = st.number_input("Введите ваш бюджет ремонта, ₽", value=520000, step=50000, key="renov_custom_val")
                    
                if renov_val != capex_renovation_val:
                    st.session_state['capex_renovation_val'] = renov_val
                    st.rerun()
                    
            with col_b2:
                st.markdown("**💻 Мебель, Техника и Оборудование:**")
                equip_pkg = st.radio(
                    "Выберите пакет техники:",
                    options=["Оптимальный до 25 мест (815 000 ₽)", "Флагманский до 45 мест (1 500 000 ₽)", "Свой бюджет (ввести вручную)"],
                    key="equip_radio_pkg"
                )
                if equip_pkg == "Оптимальный до 25 мест (815 000 ₽)":
                    equip_val = 815000
                elif equip_pkg == "Флагманский до 45 мест (1 500 000 ₽)":
                    equip_val = 1500000
                else:
                    equip_val = st.number_input("Введите ваш бюджет на технику, ₽", value=815000, step=50000, key="equip_custom_val")
                    
                if equip_val != capex_equipment_val:
                    st.session_state['capex_equipment_val'] = equip_val
                    st.rerun()
            
            # Donut chart of CAPEX
            st.markdown("<br>", unsafe_allow_html=True)
            capex_structure = {
                'Паушальный взнос + Юр.сопр': capex_m0_franchise,
                'Ремонт и Брендирование': capex_m0_renovation,
                'Оборудование, мебель, ПК': capex_m0_equipment,
                'Стартовый резерв (Аренда/OPEX)': start_opex_total
            }
            ...
            fig_capex_donut.update_layout(
                title=f"Итого стартовые инвестиции (Месяц 0): {total_capex_m0 + start_opex_total:,.0f} ₽".replace(",", " "),
        ...
        with col_right:
            ...
            <div style='background-color: #F8F9FA; padding: 20px; border-radius: 8px; border-left: 5px solid #BEAF87;'>
                ...
                <p style='font-size: 13px; color: #777779; line-height: 1.5;'>
                    <b>Сценарий разговора (скрипт):</b><br>
                    <i>"Стартовый пакет ремонта 'Стандарт C21' составляет {capex_m0_renovation:,.0f} ₽. Мы выверено подбираем материалы и элементы мебели для того, чтобы уложиться в этот бюджет. Также мы заложили резерв оплат {start_rent:,.0f} ₽ на аренду и связь на старте запуска офиса. Давайте настроим команду, чтобы запустить продажи."</i>
                </p>
            </div>
            """, unsafe_allow_html=True)
