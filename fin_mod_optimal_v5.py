...
for i in range(24):
    ...
    reklama_list.append(base_reklama[i] * leads_mult * (marketing_mult / 100.0))
    ...
    total_opex = (office_rent_custom * (base_rent_list[i] / 150000.0) + base_internet[i] + base_mobile[i] + base_kanc[i] + 
                  base_reklama[i] * leads_mult * (marketing_mult / 100.0) + base_hh[i] + base_buh[i] + base_bank[i] + base_cleaning[i] + base_gsm[i] + 
...
capex_m0_franchise = pau_fee_val + 17000
capex_m0_renovation = capex_renovation_val
capex_m0_equipment = capex_equipment_val
total_capex_m0 = capex_m0_franchise + capex_m0_renovation + capex_m0_equipment

cum_flow = []
running_sum = -total_capex_m0 - office_rent_custom
...
with tab2:
    ...
    p_and_l_decomposed['Старт'] = [
        ...
        f"{capex_renovation_val:,.0f} ₽".replace(",", " "), # Ремонт
        f"{capex_equipment_val:,.0f} ₽".replace(",", " "), # Оборудование
